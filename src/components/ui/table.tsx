...
export const TH = React.forwardRef<HTMLTableCellElement, React.ThHTMLAttributes<HTMLTableCellElement>>(
  ({ className, ...props }, ref) => (
    <th
      ref={ref}
      className={cn(
        'h-11 whitespace-nowrap border-r px-4 text-left align-middle font-medium text-muted-foreground last:border-r-0',
        className,
      )}
      {...props}
    />
...
export const TD = React.forwardRef<HTMLTableCellElement, React.TdHTMLAttributes<HTMLTableCellElement>>(
  ({ className, ...props }, ref) => (
    <td ref={ref} className={cn('border-r px-4 py-3 align-middle last:border-r-0', className)} {...props} />
  ),
);
TD.displayName = 'TD';
